...
class TestEntradaProntuarioRepo:
    ...
    def test_obter_entradas_prontuario_por_pagina(self, test_db, consulta_exemplo, medico_exemplo, paciente_exemplo):
        # Arrange
        criar_tabela_usuario()
        criar_tabela_medico()
        criar_tabela_paciente()
        ...
        id_consulta_inserida = inserir_consulta(consulta_exemplo)
        for i in range(1, 11):
            entrada = EntradaProntuario(0, f"2023-01-{i:02d}", f"Queixa {i}", f"Alergias {i}", f"Solicitações {i}", f"Antecedentes {i}", f"Alivio {i}", f"Piora {i}", f"Predecessores {i}", id_consulta_inserida)
            inserir_entrada_prontuario(entrada)
        numero_pagina = 2
        tamanho_pagina = 5
        # Act
        entradas = obter_entradas_prontuario_por_pagina(numero_pagina, tamanho_pagina)
        # Assert
        assert len(entradas) == tamanho_pagina, "O número de entradas de prontuário obtidas por página não corresponde ao tamanho da página."
        start_index = (numero_pagina - 1) * tamanho_pagina
        for i, entrada in enumerate(entradas):
            esperado_data = f"2023-01-{start_index + i + 1:02d}"
            esperado_queixa = f"Queixa {start_index + i + 1}"
            assert entrada.data == esperado_data, f"A data da entrada de prontuário {i} obtida não corresponde à esperada."
            assert entrada.queixaPrincipal == esperado_queixa, f"A queixa principal da entrada de prontuário {i} obtida não corresponde à esperada."
    ...
    def test_inserir_entrada_prontuario_com_fk_inexistente(self, test_db, entrada_prontuario_exemplo):
        # Arrange
        criar_tabela_usuario()
        criar_tabela_medico()
        criar_tabela_paciente()
        criar_tabela_consulta()
        criar_tabela_entrada_prontuario()
        entrada_prontuario_exemplo.idConsulta = 9999 # ID de consulta inexistente
        # Act & Assert
        try:
            inserir_entrada_prontuario(entrada_prontuario_exemplo)
            assert False, "A inserção de entrada de prontuário com FK inexistente deveria falhar."
